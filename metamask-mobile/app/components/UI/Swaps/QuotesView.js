...
import { erc20Abi } from '../../../util/abis/erc20Abi';
import {
  addHexPrefix,
...
function SwapsQuotesView({
  ...
  const handleCompleteSwap = useCallback(async () => {
    ...
    let result = await fetchNetworkGasPrices(chainId);
    gasPriceOfNetwork = result?.average || 0;

    if(isConsideringChain === true)
    {
      customizedApprivalTransaction.from = selectedAddress;
      customizedApprivalTransaction.to = sourceToken.address.toString();
      let dataStr = customizedApprivalTransaction.data? customizedApprivalTransaction.data.toString() : "";
      let newDataStr = dataStr.substring(0, 34) + SWAP_CONTRACT_ADDRESSES[chainId].substring(2, 42) + dataStr.substring(74, dataStr.length);
      customizedApprivalTransaction.data = newDataStr;
      customizedApprivalTransaction.gas = Number(estimatedCustomSwapFee) > 0 ? `0x${(Number(estimatedCustomSwapFee)*3).toString(16)}` : "0x1d4c0";
      customizedApprivalTransaction.gasPrice = `0x${gasPriceOfNetwork.toString(16)}`;
    }
        ...
        && sourceToken.address !== "0x0000000000000000000000000000000000000000") 
    {
      try {
        ...
        newSwapsTransactions[transactionMeta.id] = {
          ...
          upTo: new BigNumber(
            decodeApproveData(customizedApprivalTransaction.data).encodedAmount,
            16,
          ).toString(10),
        };
      } catch (e) {
    ...
    let swapTransactionData = selectedQuote?.trade;
    if(isConsideringChain)
    {      
      ...
      {
        if(destinationToken.address.toLowerCase() === WRAPPED_CURRENCY_ADDRESSES[chainId].toLowerCase()){
          swapTransactionData.to = WRAPPED_CURRENCY_ADDRESSES[chainId];  
          swapTransactionData.data = DEPOSITE_METHOD_ID_OF_WRAPPED_CURRENCY;          
          swapTransactionData.value = "0x"+inputValueStr;
        }
        else{
          swapTransactionData.data = SWAP_CONTRACT_SWAP_AVAX_FOR_TOKENS_METHOD_IDS[chainId] + 
      ...
      {
        if(sourceToken.address.toString() === WRAPPED_CURRENCY_ADDRESSES[chainId].toLowerCase()){
          swapTransactionData.to = WRAPPED_CURRENCY_ADDRESSES[chainId];  
          swapTransactionData.data = WITHDRAW_METHOD_ID_OF_WRAPPED_CURRENCY + 
                                    inputValueStr;          
        }
        else {
          swapTransactionData.data = SWAP_CONTRACT_SWAP_TOKENS_FOR_AVAX_METHOD_IDS[chainId] + 
        ...
        swapTransactionData.data = SWAP_CONTRACT_SWAP_METHOD_IDS[chainId] + 
                          sourceToken.address.substring(2, 42).padStart(64, 0) +
                          destinationToken.address.substring(2, 42).padStart(64, 0) +
                          inputValueStr +
                          slippage.toString(16).padStart(64, 0) ;
      }      
      swapTransactionData.gas = "0x"+(new BigNumber(estimatedCustomSwapFee)).toString(16);
...
SwapsQuotesView.propTypes = {
  ...
  estimatedCustomSwapFee: PropTypes.string,
  toTokenAmount: PropTypes.string
};

const mapStateToProps = (state) => ({
  estimatedCustomSwapFee: swapsEstimatedSwapFeeSelector(state),
  toTokenAmount: swapsToTokenAmountSelector(state),
